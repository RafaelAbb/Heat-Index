...
function Stat({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div className="flex flex-col items-center gap-1 bg-slate-800/60 rounded-xl p-4 border border-slate-700/50">
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Stat icon="🌡️" label="Temperature" value={`${weather.temperature}°C`} />
        <Stat icon="💧" label="Humidity" value={`${weather.humidity}%`} />
        <Stat icon="🌬️" label="Wind" value={`${weather.windSpeed} km/h`} />
        <Stat icon="🌧️" label="Precipitation" value={`${weather.precipitation} mm`} />
      </div>
    </div>
  )
}
